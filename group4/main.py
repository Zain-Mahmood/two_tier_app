...
def homepage():
    homepage_content = f"""Homepage for AWS + Docker version from server: {server_id}
...
def remove_spartan():
    ...
    result = manage.delete_from_db(id_var)
    return f"{result}"


@app.route("/spartan", methods=["GET"])
def list_spartans():
    spartans_db = manage.display_db()
